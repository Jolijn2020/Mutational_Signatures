function Pipeline_test_2(file_path)
    ...
    output_dir = fullfile(parent_dir, ['Matrices', file_name, ext]); % Modify filename
    ...
    % Export D, lambda, D_nmf, and lambda_nmf to CSV
    exportMatricesToCSV(D, lambda, D_nmf, lambda_nmf, output_dir);
...
function exportMatricesToCSV(D, lambda, D_nmf, lambda_nmf, output_file)
    % exportAllToSingleCSV: Export all matrices to a single CSV file with labels
    % Inputs:
    %   D - Wasserstein dictionary matrix
    %   lambda - Wasserstein coefficient matrix
    %   D_nmf - KL NMF dictionary matrix
    %   lambda_nmf - KL NMF coefficient matrix
    ...
    % Write Wasserstein D
    fprintf(fid, 'Wasserstein NMF Dictionary (D)\n');
    fclose(fid); % Close to use writematrix
    writematrix(D, output_file, 'WriteMode', 'append');

    % Write Wasserstein lambda
    fid = fopen(output_file, 'a');
    fprintf(fid, '\nWasserstein NMF Coefficients (lambda)\n');
    fclose(fid);
    writematrix(lambda, output_file, 'WriteMode', 'append');

    % Write KL NMF D
    fid = fopen(output_file, 'a');
    fprintf(fid, '\nKL NMF Dictionary (D_nmf)\n');
    fclose(fid);
    writematrix(D_nmf, output_file, 'WriteMode', 'append');

    % Write KL NMF lambda
    fid = fopen(output_file, 'a');
    fprintf(fid, '\nKL NMF Coefficients (lambda_nmf)\n');
    fclose(fid);
    writematrix(lambda_nmf, output_file, 'WriteMode', 'append');

    fprintf('All matrices exported to %s\n', output_file);
end